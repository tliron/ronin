...
def configure_go(command=None):
    with current_context(False) as ctx:
        ctx.go.command = command or DEFAULT_GO_COMMAND

class _GoExecutor(ExecutorWithArguments):
    """
    Base class for `Go <https://golang.org/>`__ executors.
    """

    def __init__(self, command=None):
        super(_GoExecutor, self).__init__()
        self.command = lambda ctx: which(ctx.fallback(command, 'go.command', DEFAULT_GO_COMMAND))
        self.command_types = ['go']
    
class GoCompile(_GoExecutor):
    """
    Go compile executor.
    """
...
class GoPackage(Extension):
    ...
    def apply_to_phase(self, phase):
        #if isinstance(phase.executor, GoCompile):
