...
class Project(object):
    ...
    :ivar phases: phases
    :vartype phases: {:obj:`str`: :class:`~ronin.phases.Phase`}
    ...
    :vartype run: {:obj:`int`: [:obj:`str` or :obj:`~types.FunctionType`]}
    """
    ...
    def __init__(self,
        ...
        :type version: str or ~types.FunctionType
        ...
        :type variant: str or ~types.FunctionType
        :param input_path: override input path; defaults to the context's ``paths.input_path``
        :type input_path: str or ~types.FunctionType
        ...
        :type phases: {:obj:`str`: :class:`~ronin.phases.Phase`}
        """
        ...
        self.phases = StrictDict(phases, key_type=str, value_type='ronin.phases.Phase')
    ...
    def __unicode__(self):
        ...
        if version and variant:
            return '{name} {version} ({variant})'.format(name=name, version=version,
                                                         variant=variant)
        elif version and not variant:
            return '{name} {version}'.format(name=name, version=version)
        elif variant and not version:
            return '{name} ({variant})'.format(name=name, variant=variant)
        else:
            return name
    ...
    def get_output_path(self, output_type):
        ...
        :type output_type: str or ~types.FunctionType
    ...
    def get_phase_for(self, value, attr):
        ...
        if isinstance(value, Phase):
            ...
            p = value
            if p_name is None:
                raise ValueError('{} contains a phase that is not in the project'.format(attr))
        else:
            p_name = stringify(value)
            p = self.phases.get(p_name)
            if p is None:
                raise ValueError('"{}" in {} is not a phase in the project'.format(p_name, attr))
        return p_name, p
